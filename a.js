javascript:(function(){
  ...
  // NEW: Direct engine loader
  function loadEngine(engine){
    const u='https://raw.githubusercontent.com/A2MBD3/Aincrad/main/dynamic-bypass-by-@a2mbd3.js?t='+Date.now()+'&n=0&site='+engine;
    const lt=T.load('⏳ Loading '+engine+' engine...',0);
    fetch(u).then(r=>{if(!r.ok)throw new Error('HTTP '+r.status);return r.text()}).then(c=>{T.r(lt);T.prog('⚡ Executing '+engine+'...',2000);setTimeout(()=>{try{eval(c);T.suc('🎉 '+engine+' activated!',3000)}catch(e){T.err('❌ '+e.message,5000)}},500)}).catch(e=>{T.r(lt);T.err('❌ '+e.message,5000,{a:[{l:'🔄 Retry',cb:()=>loadEngine(engine)}]})});
  }
  ...
  function X(){
    ...
    if(B&&B!=='0'&&B!==0&&typeof B==='string'&&B!=='Abdullah'){T.info(`📌 Manual engine: ${B}`,1500);setTimeout(()=>loadEngine(B),1600);return}
    ...
    if(n==='Abdullah'||B==='Abdullah'){
      ...
      setTimeout(()=>loadEngine(abdEngine),2200);return;
    }
    
    // Step 5: Unsupported site
    if(!engine){T.warn('⚠️ Unsupported site!',4000,{ti:'Warning',a:[{l:'Continue?',cb:()=>{loadEngine('default')}}]});return}
    
    // Step 6: Numeric value
    if(typeof n==='number'&&!isNaN(n)){T.info(`🔢 Numeric value: ${n}`,2000);loadEngine(engine);return}
    
    // Step 7: Default load
    T.info(`📍 Site: ${site} | Engine: ${engine}`,2000);setTimeout(()=>loadEngine(engine),2100);
  }
